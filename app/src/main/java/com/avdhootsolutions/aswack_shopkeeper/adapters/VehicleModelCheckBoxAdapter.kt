...
class VehicleModelCheckBoxAdapter(
    private val context: Context,
    private val iCustomListListener: ICustomListListener

) : RecyclerView.Adapter<VehicleModelCheckBoxAdapter.CustomListViewHolder>() {

    private var itemList: List<BrandsTypesModels.Types.Models> = mutableListOf()
    ...
    override fun onBindViewHolder(holder: CustomListViewHolder, position: Int) {
        holder.checkBox.text = itemList[position].vehicle_model_name

        holder.checkBox.setOnCheckedChangeListener { buttonView, isChecked ->
            itemList[position].isChecked = isChecked
            iCustomListListener.onModelSelected()
        }
     ...
     */
    fun setList(itemList: List<BrandsTypesModels.Types.Models>) {
