...
class VehicleTypeCheckBoxAdapter(
    private val context: Context,
    private val iCustomListListener: ICustomListListener

) : RecyclerView.Adapter<VehicleTypeCheckBoxAdapter.CustomListViewHolder>() {

    private var itemList: List<BrandsTypesModels.Types> = mutableListOf()
    ...
    override fun onBindViewHolder(holder: CustomListViewHolder, position: Int) {
        holder.checkBox.text = itemList[position].vehicle_type_name

        holder.checkBox.setOnCheckedChangeListener { buttonView, isChecked ->
            itemList[position].isChecked = isChecked
            iCustomListListener.onTypesSelected()
        }
...
//        }
     ...
     */
    fun setList(itemList: List<BrandsTypesModels.Types>) {
