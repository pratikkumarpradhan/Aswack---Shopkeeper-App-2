...
class ForgotPasswordViewModel : ViewModel() {
     ...
     */
    fun apiForgotPassword(login: Login) {
            Log.e("request of forgot password", Gson().toJson(login))
            val loginCallBackCall: Call<Datas> = MyApp.getInstance().myApi.apiForgotPassword(login)
            loginCallBackCall.enqueue(object : Callback<Datas?> {
                override fun onResponse(call: Call<Datas?>, response: Response<Datas?>) {
                    val datas: Datas = response.body()!!
                    Log.e("response of forgot password", Gson().toJson(response.body()))
