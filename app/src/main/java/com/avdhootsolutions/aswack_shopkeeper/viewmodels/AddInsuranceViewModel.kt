...
class AddInsuranceViewModel : ViewModel() {
     ...
     */
    fun apiAddInsuranceProductWithFile(spareParts: SpareParts, context: Context) {
        Log.e("request of add Insurance product", Gson().toJson(spareParts))
...
//        }
        ...
        val vehicle_year: okhttp3.RequestBody = okhttp3.RequestBody.create(
            MultipartBody.FORM, spareParts.vehicle_year ?: "")
        ...
        val loginCallBackCall: Call<Datas> = MyApp.getInstance().getMyApi()
            .apiInsuranceProductWithImage(image1,
                image2,
                image3,
                image4,
                image5,
        ...
        loginCallBackCall.enqueue(object : Callback<Datas?> {
            override fun onResponse(call: Call<Datas?>, response: Response<Datas?>) {
                val datas: Datas = response.body()!!
                Log.e("response of Insurance product", Gson().toJson(response.body()))
                if (datas.status == true) {
                    successMessageLiveData.setValue(true)
                } else {
                    errorMessage.value = datas.message
